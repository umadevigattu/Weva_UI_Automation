...
public class WevaUtils extends BaseTest {
	...
	public void WevaLogin_muliple(SeleniumUtilities seleniumUtilities) throws Exception {
		 ...
		 * 
		 * Sheet sheet = workbook.getSheetAt(0); int lastRow = sheet.getLastRowNum();
		 * for (int rownum = 1; rownum <= sheet.getLastRowNum(); rownum++) {
		 * seleniumUtilities = new SeleniumUtilities(BrowserType.chrome,
		 * properties.getProperty("Weva_Url"), 30); CommonUtils.ExcelReader();
		 * 
		 * Row row = sheet.getRow(rownum); username =
		 * row.getCell(0).getStringCellValue(); System.out.println(username); password =
		 * row.getCell(1).getStringCellValue(); System.out.println(password);
		 * seleniumUtilities.EnterText(WevaLoginPage.getUsername(), LocatorType.xpath,
		 * username); seleniumUtilities.EnterText(WevaLoginPage.getPassword(),
		 * LocatorType.xpath, password);
		 * seleniumUtilities.Click(WevaLoginPage.getSubmit(), LocatorType.xpath);
		 * Thread.sleep(5000); seleniumUtilities.CloseWindow(); }
		 */}
